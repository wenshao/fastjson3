...
public class ClientsWriteUTF8Bytes {
    ...
    static {
        try {
            byte[] bytes;
            try (InputStream is = ClientsWriteUTF8Bytes.class.getClassLoader().getResourceAsStream("data/jjb/client.json")) {
    ...
    public void jackson(Blackhole bh) throws Exception {
        bh.consume(jackson.writeValueAsBytes(clients));
    }

    @Benchmark
    public void gson(Blackhole bh) {
        bh.consume(gson.toJson(clients).getBytes(java.nio.charset.StandardCharsets.UTF_8));
    }
