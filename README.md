...
Key optimizations:
- **DirectField optimization** — uses 4-byte field prefix + lookupswitch for O(1) field access, bypassing hash computation and linear search
...
- **Zero-copy parsing** — direct field name matching with minimal object allocation
- **Buffer pooling** — thread-local buffer reuse to minimize GC pressure

### DirectField Optimization

DirectField is an ASM-based optimization that accelerates JSON field matching by using **4-byte field prefixes** and **lookupswitch** bytecode instruction:

```
JSON: {"id":123,"name":"test","age":25}
              ↓
    Read 4 bytes: [", i, d, ] → 0x22696422
                  [", n, a, m] → 0x226e616d
              ↓
lookupswitch(prefix) → instant field dispatch
```

**Performance benefits:**
- **O(1) field matching** vs O(n) linear search
- **No runtime hash computation**
- **Inlined bytecode** (no virtual method calls)
- **Automatic fallback** to reflection for non-byte[] input

**Usage:** Automatically enabled for byte[] input. No code changes required.
