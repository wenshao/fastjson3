...
public abstract sealed class JSONParser implements Closeable
    ...
    public static final class UTF8 extends JSONParser {
        ...
        /**
         * Field name matching using fastjson2's genRead243 algorithm.
         * Uses 4-byte prefix for O(1) dispatch, then exact matching within the group.
         *
         * <p>WARNING: 性能敏感方法，这是 fastjson2 最关键的优化算法之一。</p>
         *
         * @param matcher the Prefix243Matcher with pre-computed prefix groups
         * @return the matched FieldReader, or null if not found
         */
        public com.alibaba.fastjson3.reader.FieldReader readFieldNameMatch243(
                com.alibaba.fastjson3.reader.Prefix243Matcher matcher) {
            final byte[] b = this.bytes;
            int off = this.offset;
            final int e = this.end;
            ...
            // Fast path: use 4-byte prefix matching
            if (off + 3 < e) {
                // Read 4-byte prefix (including opening quote)
                int prefix = ((b[off - 1] & 0xFF) << 24)
                            | ((b[off] & 0xFF) << 16)
                            | ((b[off + 1] & 0xFF) << 8)
                            | (b[off + 2] & 0xFF);

                // Binary search for prefix group
                int idx = matcher.binarySearchPrefix(prefix);
                if (idx >= 0) {
                    // Check all readers in this prefix group
                    com.alibaba.fastjson3.reader.Prefix243Matcher.PrefixGroup group = matcher.getGroup(idx);
                    for (com.alibaba.fastjson3.reader.FieldReader reader : group.readers) {
                        // Match against primary field name
                        if (matchFieldName243(b, off, e, reader.fieldName)) {
                            off += reader.fieldName.length();
                            if (off < e && b[off] == '"') {
                        ...
                        for (String alt : reader.alternateNames) {
                            if (matchFieldName243(b, off, e, alt)) {
                                off += alt.length();
                                if (off < e && b[off] == '"') {
                                    off++; // skip closing quote
                                    // Skip whitespace
            ...
            // No match: scan to closing quote (unknown field)
            while (off < e) {
                byte c = b[off];
                if (c == '"') {
                    off++;
                ...
                if (c == '\\') {
                    ...
                    this.offset++;
                    // Return null (unknown field with escapes)
                    return null;
                }
                off++;
            }
        ...
        /**
         * Fast field name matching for 243 algorithm.
         * Compares UTF-8 bytes directly without String allocation.
         */
        private static boolean matchFieldName243(byte[] input, int offset, int end, String expectedName) {
            int len = expectedName.length();
            if (offset + len > end) {
                return false;
            }

            // Fast path: compare byte by byte
            // For ASCII names (most common), this is very fast
            for (int i = 0; i < len; i++) {
                char c = expectedName.charAt(i);
                if (c > 127) {
                    // Non-ASCII: need proper UTF-8 comparison
                    return matchFieldName243UTF8(input, offset, end, expectedName);
                }
                if (input[offset + i] != (byte) c) {
                    return false;
                }
            }
            return true;
        ...
        /**
         * UTF-8 field name matching for non-ASCII characters.
         */
        private static boolean matchFieldName243UTF8(byte[] input, int offset, int end, String expectedName) {
            byte[] expected = expectedName.getBytes(java.nio.charset.StandardCharsets.UTF_8);
