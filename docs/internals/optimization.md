...
|----------|----------|------|----------|
| SWAR 并行处理 | ~10% | ⭐⭐⭐ | 字符串处理 |
| ASM 字节码生成 | 10-13% 慢于反射 | ⭐⭐⭐⭐ | 跨 ClassLoader 兼容 |
| Unsafe 直接操作 | ~5% | ⭐⭐⭐ | 高频路径 |
| 字段预编码 | ~5% | ⭐⭐ | 固定格式 |
| 缓冲区池化 | ~3% | ⭐⭐ | IO 操作 |
| 单遍检查复制 | ~8% | ⭐⭐⭐ | 字符串复制 |
...
ASM 生成的方法体将所有字段展开，超出 JIT 内联预算（~325 bytes），关键方法（如
`writeNameString`、`readStringOff`）无法被内联，实际上比反射路径慢 10-13%。

| 方法 | 相对性能 | 说明 |
|------|----------|------|
| 反射 Reader | **100%** | 基准（JIT 深度内联，最快） |
| ASM Reader | ~87% | 方法体过大，JIT 内联受限 |
| 反射 Writer | **100%** | 基准（JIT 深度内联，最快） |
| ASM Writer | ~87% | 方法体过大，JIT 内联受限 |

### 你可以学到什么

1. **代码生成 ≠ 更快** - JIT 对反射路径的紧凑循环做了深度优化，手动展开反而超出内联预算
2. **JIT 内联预算** - FreqInlineSize ~325 字节码是硬边界，ASM 生成的大方法阻碍内联
3. **架构选择** - ASM 的价值在于跨 ClassLoader 兼容性，而非性能提升
