...
| **Java 版本** | Java 6+ | Java 21+ | ⚠️ 需要升级 JDK |
...
1. **直接迁移** - fastjson 1.x → fastjson3（推荐，如果你能升级到 Java 21）
2. **分步迁移** - fastjson 1.x → fastjson2 → fastjson3

> 💡 **提示**：fastjson2 是一个过渡版本，兼容 Java 8+。如果你暂时无法升级到 Java 21，可以先迁移到 fastjson2。
...
| **fastjson3** | Java 21+ | `com.alibaba.fastjson3` | 现代 API，Jackson 风格，性能最优 |
...
A: 是的，fastjson3 需要 Java 21+。如果不能升级，继续使用 fastjson 1.x 或 fastjson2。
